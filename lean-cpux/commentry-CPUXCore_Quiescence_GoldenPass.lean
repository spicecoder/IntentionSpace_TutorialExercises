...
/- ============================================================
   10. Human-readable executable report with running commentary
   ============================================================ -/

def yesNo (b : Bool) : String :=
  if b then "true" else "false"

def tvLabel : TV → String
  | .Y  => "Y"
  | .N  => "N"
  | .UN => "UN"

def pulseKeyLabel (k : PulseKey) : String :=
  s!"{k.name}={tvLabel k.tv}"

def missingPulseKeys (f : Field α) (s : SyncTest) : List PulseKey :=
  s.requiredPulses.filter (fun k => !(hasPulseKey f k))

def joinLabels (xs : List String) : String :=
  String.intercalate ", " xs

def missingPulseText (f : Field α) (s : SyncTest) : String :=
  let xs := (missingPulseKeys f s).map pulseKeyLabel
  if xs.isEmpty then "none" else joinLabels xs

def admissionReason (f : Field Data) (dn : DN Data) : String :=
  if enabled f dn.sync then
    s!"ENABLED because Intention '{dn.sync.intention}' is present and all required Pulses are present"
  else
    let intentionMissing := !(hasIntention f dn.sync.intention)
    let missing := missingPulseKeys f dn.sync
    if intentionMissing && !missing.isEmpty then
      s!"BLOCKED because Intention '{dn.sync.intention}' is absent and required Pulse(s) are missing: {missingPulseText f dn.sync}"
    else if intentionMissing then
      s!"BLOCKED because Intention '{dn.sync.intention}' is absent"
    else
      s!"BLOCKED because required Pulse(s) are missing: {missingPulseText f dn.sync}"

def printFieldSummary (f : Field Data) : IO Unit := do
  let is := if f.intentions.isEmpty then "none" else String.intercalate ", " f.intentions
  let ps :=
    if f.pulses.isEmpty then
      "none"
    else
      String.intercalate ", " (f.pulses.map (fun p => pulseKeyLabel p.key))
  IO.println s!"  Field Intentions : {is}"
  IO.println s!"  Field Pulses     : {ps}"

def printDNReason (f : Field Data) (dn : DN Data) : IO Unit := do
  IO.println s!"  {dn.id} : {admissionReason f dn}"

def printState (label : String) (f : Field Data) : IO Unit := do
  IO.println s!"\n{label}"
  printFieldSummary f
  IO.println "  Admission commentary:"
  printDNReason f dnPopulation
  printDNReason f dnTraffic
  printDNReason f dnHospital
  if quiescent f allDNs then
    IO.println "  Quiescent : true — the current Field satisfies no configured DN SyncTest."
  else
    IO.println "  Quiescent : false — at least one configured DN is currently admissible."

def demo : IO Unit := do
  IO.println "CPUX / LEAN DESIGN ANALYSIS"
  IO.println "==========================="
  IO.println "The commentary below explains each DN status from the relation"
  IO.println "between the current Field and that DN's SyncTest."
  IO.println ""

  printState "F0 — initial Signal received" F0
  IO.println "  Commentary:"
  IO.println "    The initial Signal places region, year and scenario in the Field."
  IO.println "    This satisfies Population's SyncTest, so Population may execute."
  IO.println "    Traffic and Hospital remain blocked because their prerequisite"
  IO.println "    result Pulses have not yet been emitted."

  printState "F1 — after Population" F1
  IO.println "  Commentary:"
  IO.println "    Population has emitted population_total and population_spread."
  IO.println "    Because Population uses pull mode, its original input Pulses were"
  IO.println "    consumed, preventing Population from immediately re-firing."
  IO.println "    population_spread now satisfies Traffic's SyncTest."
  IO.println "    Hospital remains blocked because traffic_volume is still absent."

  printState "F2 — after Traffic" F2
  IO.println "  Commentary:"
  IO.println "    Traffic has consumed population_spread and emitted traffic_volume."
  IO.println "    population_total remains available in the Field."
  IO.println "    Together, population_total and traffic_volume satisfy Hospital's"
  IO.println "    SyncTest, making Hospital the admissible next computation."

  printState "F3 — after Hospital" F3
  IO.println "  Commentary:"
  IO.println "    Hospital has consumed its prerequisite Pulses and emitted the final"
  IO.println "    result Pulses. No configured DN now has a satisfied SyncTest."
  IO.println "    The Field is therefore quiescent."

  IO.println "\nLogical-data handle"
  IO.println "  Commentary:"
  IO.println "    The Pulse 'population_spread=Y' is the logical handle used by"
  IO.println "    SyncTest, while its response carries the associated data."
  ...
  IO.println "  Commentary:"
  IO.println "    These relations are derived from emitted Pulse handles and"
  IO.println "    downstream SyncTest requirements, rather than an explicit DAG."
  ...
  IO.println "    No DN is admissible from F3."
  IO.println s!"  F4 next quiescent pass  : {yesNo (quiescent F4 allDNs)}"
  IO.println "    Rechecking produces no new admissible DN."
  IO.println s!"  Field unchanged          : {yesNo (F3 == F4)}"
  IO.println "    The perceived operational state is stable across the two passes."
  IO.println s!"  GOLDEN PASS              : {yesNo (goldenPass F3 F4 allDNs)}"
  IO.println "    Golden Pass therefore represents stable quiescence, not merely"
  IO.println "    a momentary absence of execution."

  IO.println "\nQuiescence quality"
  IO.println s!"  F3 successful quiescence  : {yesNo (successfulQuiescence F3 allDNs simulationComplete)}"
  IO.println "    The Field is quiescent and the declared completion condition holds."
  IO.println s!"  Unknown-case quiescence   : {yesNo (quiescent F_unresolved allDNs)}"
  IO.println "    No available DN SyncTest admits the unresolved Field."
  IO.println s!"  Unknown-case complete     : {yesNo (simulationComplete F_unresolved)}"
  IO.println "    The application-level completion condition is still false."
  IO.println s!"  UNRESOLVED QUIESCENCE     : {yesNo (unresolvedQuiescence F_unresolved allDNs simulationComplete)}"
  IO.println "    This exposes the boundary of currently configured capability:"
  IO.println "    the Field is stable, but no configured DN knows how to progress it."
